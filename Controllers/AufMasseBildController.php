 ...
 * The AufMasseBild controller entity class.
 * @author
 *    Guy Bami Watcho
 */
include_once 'BaseController.php';

class AufMasseBildController extends BaseController {

    private $aufMasseBild = null;

    /**
     * Constructor of the AufMasseBild controller 
    ...
    function __construct($userAction, $userRoles = array()) {
        ...
        $this->aufMasseBild = new AufMasseBild();
    }

    /**
     * Gets all AufMasseBild entities
     * @return mixed object having all AufMasseBild entities
     *    or string with the Exception details if error occured
     */
    public function getAllAufMasseBilds() {

        $itemSperator = ",";
        $entitiesList = "";
        $resultObject = $this->aufMasseBild->selectAllAufMasseBilds();
     ...
     *    or string with the Exception details if error occured
     */
    public function insertNewAufMasseBild($jsonData) {
        ...
        $resultObject = $this->aufMasseBild->insertNewAufMasseBild(
    ...
    /**
     * Updates AufMasseBild entity via POST request
     ...
     *    or string with the Exception details if error occured
     */
    public function updateAufMasseBild($bildId, $updateMode, $jsonData) {
        ...
        if ($updateMode == "allFields") {
            ...
            $resultObject = $this->aufMasseBild->updateAufMasseBildDetails(
                    $bildId
                    , $formJsonValues['aufmasseId']
                    , $formJsonValues['dateiName']
            );
            ...
            $resultObject = $this->aufMasseBild->updateAufMasseBildEntityField($fieldName, $keyFieldValue, $newFieldValue);
            if (is_string($resultObject)) {
                //error occured
                $postbackData = Utils::formatJsonErrorMessage($resultObject);
     ...
     * @return mixed
     */
    public function deleteAufMasseBilds($jsonData) {

        // delete entity using model
        $resultObject = $this->aufMasseBild->deleteSelectedAufMasseBilds($jsonData);
        if (is_bool($resultObject)) {
            if ($resultObject == true) {
    ...
    public function deleteAllAufMasseBilds() {
        ...
        $resultObject = $this->aufMasseBild->deleteAllAufMasseBilds();
        if (is_bool($resultObject)) {
            if ($resultObject == true) {
                $postbackData = Utils::formatJsonResultMessage(Common::$DELETE_SUCCESSFUL);
            } else {
    ...
    public function getDynamicPageContent() {
        ...
        switch ($this->getUserAction()) {
            case "getAllItems":
                $postbackData = $this->getAllAufMasseBilds();
                break;
            case "addNewItem":
            case "insertNewItem":
                $jsonData = array();
                ...
                $postbackData = $this->insertNewAufMasseBild($jsonData);
                break;
            case "updateItem":
                ...
                $bildId = $_POST['bildId'];
                $jsonData = $_POST['formValues'][0];
                $postbackData = $this->updateAufMasseBild($bildId, $updateMode, $jsonData);
                break;
            case "deleteItem":
                $jsonData = array();
                ...
                $postbackData = $this->deleteAufMasseBilds($jsonData);
                break;
            case "deleteAllItems":
                $postbackData = $this->deleteAllAufMasseBilds();
                break;
            case "editDetails":
                $postbackData = "Undefined Content";
                if (isset($_POST['bildId'])) {
                    $bildId = $_POST['bildId'];
                    $resultObject = $this->aufMasseBild->getAufMasseBildDetails($bildId);
                }
                if (is_string($resultObject)) {
                    $postbackData = Utils::formatJsonErrorMessage($resultObject);
                } else if (is_array($resultObject)) {
...
if (!isset($userAction)) {
    ...
    $aufMasseBildController = new AufMasseBildController($userAction);
    $aufMasseBildController->setUserAction($userAction);
    $postbackContent = $aufMasseBildController->getDynamicPageContent();
