...
class TaskLogTimeController extends BaseController {
        ...
        if (is_string($resultObject)) {
            ...
            for ($i = 0; $i < count($resultObject); $i++) {
                $entitiesList .=
                        ...
                        . ',"startTime":"' . Utils::formatShortTime($resultObject[$i]["startTime"]) . '"'
                        . ',"endTime":"' . Utils::formatShortTime($resultObject[$i]["endTime"]) . '"'
                        . ',"summary":"' . $resultObject[$i]["summary"] . '"'
                        . ',"date":"' . Utils::formatShortDate($resultObject[$i]["date"]) . '"'
                        // foreigns
                        . ',"loginName":"' . $resultObject[$i]["loginName"] . '"'
                        . ',"workedHours":"' . $resultObject[$i]["workedHours"] . '"'
    ...
    public function updateTaskLogTime($taskLogTimeId, $updateMode, $jsonData) {
        ...
        if ($updateMode == "allFields") {
            ...
            if (is_bool($resultObject) || is_int($resultObject)) {
                if ($resultObject == true || $resultObject == 1) {
    ...
    public function deleteTaskLogTimes($jsonData) {
        ...
        $resultObject = $this->taskLogTime->deleteSelectedTaskLogTimes($jsonData[0]);
        if (is_bool($resultObject) || is_int($resultObject)) {
    ...
    // custom
    public function getAllTaskLogTimesByUser($loginName) {
        ...
        if (is_string($resultObject)) {
            ...
            for ($i = 0; $i < count($resultObject); $i++) {
                if($resultObject[$i]["loginName"] == $loginName){
                    $entitiesList .=
                        '{"taskLogTimeId":"' . $resultObject[$i]["taskLogTimeId"] . '"'
                        . ',"userId":"' . $resultObject[$i]["userId"] . '"'
                        . ',"taskId":"' . $resultObject[$i]["taskId"] . '"'
    ...
    public function getUserTaskLogTimes($userId, $taskId) {

        $itemSperator = ",";
        $entitiesList = "";
        $resultObject = $this->taskLogTime->selectUserTaskLogTimes($userId, $taskId);
        if (is_string($resultObject)) {
            return Utils::formatJsonErrorMessage($resultObject);
        } else if (is_array($resultObject)) {
            for ($i = 0; $i < count($resultObject); $i++) {
    ...
    public function getDynamicPageContent() {
        ...
        switch ($this->getUserAction()) {
            ...
            case "cancelChanges":
                ...
                break;
            case "getAllTaskLogTimesByUser":
                if (isset($_POST['loginName'])) {
                    $loginName = $_POST['loginName'];
                     
                    $postbackData = $this->getAllTaskLogTimesByUser($loginName);
                }
                 
                break;
            case "getUserTaskLogTimes":
                //  getUserTaskLogTimes($userId, $taskId)
                if (isset($_POST['userId'])) {
                    $userId = $_POST['userId'];
                    $taskId = $_POST['taskId'];
                    $postbackData = $this->getUserTaskLogTimes($userId, $taskId);
                }
...
function testAddNewItem() {
    ...
    $startTime = $now;
    $endTime = date("Y-m-d H:i:s", strtotime("+5 hours", strtotime($now)));
    $title = "Title - Task:4 - " . uniqid();
    $summary = "Bericht - Task:4 - " . uniqid();
...
function testUpdateItem() {
    ...
    $now = date("Y-m-d H:i:s");
    $startTime = $now;
    $endTime = date("Y-m-d H:i:s", strtotime("+2 hours", strtotime($now)));
    $title = "Title - Task:3 - UPDATED - " . uniqid();
    $summary = "Bericht - Task:3 - UPDATED";
     
    $jsonRaw = '{"userAction":"updateItem", "updateMode":"allFields", "taskLogTimeId":"5", "formValues":'
            . '[{"userId":"1"'
...
function testGetUserTaskLogTimes() {

    $jsonRaw = '{"userAction":"getUserTaskLogTimes", "userId":"1", "taskId":"4"}';
    if (is_string($jsonRaw)) {
        $_POST = json_decode($jsonRaw, true);
    } else {
        $_POST = $jsonRaw;
...
function tesGetAllTaskLogTimesByUser() {

    // getAllTaskLogTimesByUser($loginName)
    $jsonRaw = '{"userAction":"getAllTaskLogTimesByUser", "loginName":"administrator"}';
    if (is_string($jsonRaw)) {
        $_POST = json_decode($jsonRaw, true);
    } else {
        $_POST = $jsonRaw;
...
//testGetUserTaskLogTimes();
//tesGetAllTaskLogTimesByUser();
