 ...
 * The Order controller entity class.
 * @author
 *    Guy Bami Watcho
 */
...
class OrderController extends BaseController {
    ...
    /**
     * Constructor of the Order controller 
    ...
    /**
     * Gets all Order entities
     * @return mixed object having all Order entities
    ...
    public function getAllOrders() {
        ...
        $itemSperator = ",";
        $entitiesList = "";
        $resultObject = $this->order->selectAllOrders();
        if (is_string($resultObject)) {
            ...
            for ($i = 0; $i < count($resultObject); $i++) {
                $entitiesList .=
                        '{"orderId":"' . $resultObject[$i]["orderId"] . '"'
     ...
     *    or string with the Exception details if error occured
     */
    public function insertNewOrder($jsonData) {
        ...
        $resultObject = $this->order->insertNewOrder(
    ...
    /**
     * Updates Order entity via POST request
     ...
     *    or string with the Exception details if error occured
     */
    public function updateOrder($orderId, $updateMode, $jsonData) {
        ...
        if ($updateMode == "allFields") {
            ...
            $resultObject = $this->order->updateOrderDetails(
                    $orderId
                    , $formJsonValues['title']
                    , $formJsonValues['projectId']
                    , $formJsonValues['date']
            ...
            $resultObject = $this->order->updateOrderEntityField($fieldName, $keyFieldValue, $newFieldValue);
            if (is_string($resultObject)) {
                //error occured
                $postbackData = Utils::formatJsonErrorMessage($resultObject);
     ...
     * @return mixed
     */
    public function deleteOrders($jsonData) {

        // delete entity using model
        $resultObject = $this->order->deleteSelectedOrders($jsonData);
        if (is_bool($resultObject)) {
            if ($resultObject == true) {
    ...
    public function deleteAllOrders() {
        ...
        $resultObject = $this->order->deleteAllOrders();
        if (is_bool($resultObject)) {
            if ($resultObject == true) {
                $postbackData = Utils::formatJsonResultMessage(Common::$DELETE_SUCCESSFUL);
            } else {
    ...
    public function getDynamicPageContent() {
        ...
        switch ($this->getUserAction()) {
            case "getAllItems":
                $postbackData = $this->getAllOrders();
                break;
            case "addNewItem":
            case "insertNewItem":
                $jsonData = array();
                ...
                $postbackData = $this->insertNewOrder($jsonData);
                break;
            case "updateItem":
                ...
                $jsonData = $_POST['formValues'][0];
                $postbackData = $this->updateOrder($orderId, $updateMode, $jsonData);
                break;
            case "deleteItem":
                $jsonData = array();
                ...
                $postbackData = $this->deleteOrders($jsonData);
                break;
            case "deleteAllItems":
                $postbackData = $this->deleteAllOrders();
                break;
            case "editDetails":
                $postbackData = "Undefined Content";
                if (isset($_POST['orderId'])) {
                    $orderId = $_POST['orderId'];
                    $resultObject = $this->order->getOrderDetails($orderId);
                }
                if (is_string($resultObject)) {
                    $postbackData = Utils::formatJsonErrorMessage($resultObject);
                } else if (is_array($resultObject)) {
...
if (!isset($userAction)) {
    ...
    $orderController = new OrderController($userAction);
    $orderController->setUserAction($userAction);
    $postbackContent = $orderController->getDynamicPageContent();
